...
public class bl_AllOptionsPro : MonoBehaviour
{
    ...
    [SerializeField, /*Range(0, 8)*/ Range(0, 5)]
    private int DefaultQuality = 5; //3;
    [SerializeField, Range(0, 21)]
    private int DefaultResolution = 10; //Screen.resolutions.Length - 1;
    ...
    [SerializeField] private Text MotionBlurText;
    private int CurrentMB;
    ...
    [SerializeField] private Text VolumeSFXText;
    ...
    void Start()
    {
        FMODUnity.RuntimeManager.GetVCA("vca:/Master").setVolume(1f);
    ...
    public void ToggleMotionBlur(bool active)
    {
        CurrentMB = active ? 1 : 0;
        MotionBlurText.text = active ? "ENABLE" : "DISABLE";
        ...
        if (volume != null)
        {
            ...
            volume.profile.TryGetSettings(out mbSettings);
            if (mbSettings != null)
                mbSettings.enabled.value = active;
        }
    }
    ...
    {
        _masterVolume = VolumeMasterSlider.value;
        //FMODUnity.RuntimeManager.GetVCA("vca:/Master").setVolume(_masterVolume);
        VolumeMasterText.text = (_masterVolume * 100).ToString("00") + "%";

        _musicVolume = VolumeMusicSlider.value;
        FMODUnity.RuntimeManager.GetVCA("vca:/Music").setVolume(_musicVolume);
        VolumeMusicText.text = (_musicVolume * 100).ToString("00") + "%";

        _SFXVolume = VolumeSFXSlider.value;
        FMODUnity.RuntimeManager.GetVCA("vca:/SFX").setVolume(_SFXVolume);
        VolumeSFXText.text = (_SFXVolume * 100).ToString("00") + "%";
    }
    public void UpdateVolume(float vMaster, float vMusic, float vSFX)
    {
        _masterVolume = vMaster;
        //FMODUnity.RuntimeManager.GetVCA("vca:/Master").setVolume(_masterVolume);
        VolumeMasterText.text = (_masterVolume * 100).ToString("00") + "%";
    ...
    public void ApplyResolution()
    {
        ...
        Screen.SetResolution(Screen.resolutions[Screen.resolutions.Length - 1].width, Screen.resolutions[Screen.resolutions.Length - 1].height, apply);
#endif
    }
    ...
    void LoadAndApply()
    {
        ...
        CurrentRS = PlayerPrefs.GetInt(AllOptionsKeyPro.Resolution, /*DefaultResolution*/  0);
        ...
        CurrentMB = PlayerPrefs.GetInt(AllOptionsKeyPro.MotionBlur, 1);
        ...
        _masterVolume = PlayerPrefs.GetFloat(AllOptionsKeyPro.MasterVolume, 1);
        _musicVolume = PlayerPrefs.GetFloat(AllOptionsKeyPro.MusicVolume, 1);
        _SFXVolume = PlayerPrefs.GetFloat(AllOptionsKeyPro.SFXVolume, 1);
        ...
        useFullScreen = (PlayerPrefs.GetInt(AllOptionsKeyPro.ResolutionMode, 1) == 1) ? true : false;
        ...
        UpdateVolume(_masterVolume, _musicVolume, _SFXVolume);
        VolumeMasterSlider.value = _masterVolume;
        VolumeMusicSlider.value = _musicVolume;
        VolumeSFXSlider.value = _SFXVolume;
        ...
        MotionBlurText.text = (CurrentMB == 1) ? "ENABLE" : "DISABLE";
    ...
    public void SaveOptions()
    {
        ...
        PlayerPrefs.SetInt(AllOptionsKeyPro.MotionBlur, CurrentMB);
        PlayerPrefs.SetFloat(AllOptionsKeyPro.Volumen, _volumen);
        PlayerPrefs.SetFloat(AllOptionsKeyPro.MasterVolume, _masterVolume);
        PlayerPrefs.SetFloat(AllOptionsKeyPro.MusicVolume, _musicVolume);
        PlayerPrefs.SetFloat(AllOptionsKeyPro.SFXVolume, _SFXVolume);
